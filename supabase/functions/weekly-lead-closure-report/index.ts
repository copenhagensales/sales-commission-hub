 ...
 * emnets status til "Depleted", når loftet af opkaldsforsøg er nået. Adversus
 * har ingen tilsvarende markering (undersøgt: hverken emne- eller opkaldsdata
 * indeholder en lukkeårsag), og MCR opgøres derfor ikke for Adversus.
 */
...
type EnreachFacts = { campaignId: string; status: string; closure: string; user: string; day: string };
...
async function processEnreachTask(
  ...
  const scanned = await streamEnreachWeek(access, weekStart, (lead) => {
    ...
    // Max Call Reach er dialerens egen markering: Enreach sætter status
    // "Depleted", når loftet af opkaldsforsøg er nået. Emnet er ikke afsluttet
    // af en sælger og tælles derfor uden sælgerfilter, under sin egen nøgle og
    // uden for tragten (hverken Leads behandlet, kvalificerede eller hitrate).
    if (lead.status === ENREACH_MCR_STATUS) {
      const mcrKey = `${weekStart}|enreach|${lead.campaignId}||${MCR_STATUS}`;
