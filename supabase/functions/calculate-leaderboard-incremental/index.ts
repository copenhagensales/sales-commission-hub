...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Unified fetch - no separate FM query needed
    const salesWithItems = await fetchAllSalesWithItems(supabase, earliestStart.toISOString(), latestEnd.toISOString());

    console.log(`[calculate-leaderboard-incremental] Loaded ${salesWithItems.length} total sales (TM + FM unified)`);
