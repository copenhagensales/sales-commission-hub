CREATE OR REPLACE FUNCTION public.get_client_daily_sales_totals(
  p_client_id uuid,
  p_start date,
  p_end date
)
RETURNS TABLE(sale_date date, quantity bigint, sale_count bigint)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.get_client_daily_sales_totals(uuid, date, date) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_client_daily_sales_totals(uuid, date, date) TO service_role;