ALTER TABLE public.employee_profile_stats
  ADD COLUMN IF NOT EXISTS current_streak_days integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS current_streak_start date NULL;

COMMENT ON COLUMN public.employee_profile_stats.current_streak_days IS
  'Igangvaerende stribe uden nuldag (samme regel som longest_streak_days). Kun visning paa spillerprofilkortet.';
