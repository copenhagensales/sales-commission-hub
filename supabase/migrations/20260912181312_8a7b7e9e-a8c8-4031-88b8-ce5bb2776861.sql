-- Arbejdsdag 1 = det seneste af ansaettelsesdato og foerste salg paa kampagnen.
CREATE OR REPLACE FUNCTION public.ramp_day1(p_employee_id uuid, p_campaign uuid)
RETURNS date
LANGUAGE sql
...
BEGIN
  ...
  WITH sellers AS (
    -- Dag 1 = greatest(ansaettelsesdato, foerste salg paa kampagnen).
    SELECT e.id,
           e.employment_end_date,
           lower(coalesce(e.work_email, e.private_email)) AS email,
           greatest(e.employment_start_date, fs.first_sale) AS start_date
    FROM public.employee_master_data e
    CROSS JOIN LATERAL (
...
BEGIN
  ...
  WITH sellers AS (
    -- Dag 1 = greatest(ansaettelsesdato, foerste salg paa kampagnen).
    SELECT e.id,
           greatest(e.employment_start_date, fs.first_sale) AS sd,
...
BEGIN
  WITH enrolled AS (
    -- Dag 1 hentes fra den faelles definition, saa flag altid maales korrekt.
    SELECT en.employee_id, en.client_campaign_id, en.curve_version,
           coalesce(public.ramp_day1(en.employee_id, en.client_campaign_id), en.start_date) AS start_date,
           lower(coalesce(e.work_email, e.private_email)) AS email
...
BEGIN
  ...
  candidates AS (
    ...
    SELECT DISTINCT ON (e.id)
           ...
           greatest(e.employment_start_date,
                    (sa.sale_datetime AT TIME ZONE 'Europe/Copenhagen')::date) AS start_date,
           l.curve_version
    FROM public.employee_master_data e
    ...
    WHERE e.employment_start_date IS NOT NULL
      ...
      AND public.ramp_workday_no(
            greatest(e.employment_start_date,
                     (sa.sale_datetime AT TIME ZONE 'Europe/Copenhagen')::date),
...
REVOKE ALL ON FUNCTION public.ramp_day1(uuid, uuid) FROM PUBLIC, anon;