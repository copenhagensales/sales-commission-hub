...
BEGIN
  ...
  rolling AS (
    SELECT v.*
    FROM valid_spells v
    WHERE v.start_date > (v_as_of - v_horizon)
  ),
  rolling_team AS (
    SELECT team_key,
           count(*)::int AS starters,
           count(*) FILTER (WHERE exit_date IS NOT NULL AND exit_day BETWEEN 0 AND v_horizon)::int AS exits
  ...
  SELECT jsonb_build_object(
    ...
    'rolling_window', jsonb_build_object(
      'days', v_horizon,
      'window_start', (v_as_of - v_horizon + 1),
      'total', (SELECT to_jsonb(x) FROM rolling_total x),
      'teams', (SELECT coalesce(jsonb_agg(to_jsonb(x)), '[]'::jsonb) FROM rolling_team x)
    ),
