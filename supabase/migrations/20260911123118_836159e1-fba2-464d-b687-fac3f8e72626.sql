...
BEGIN
  ...
  -- Set-based on purpose: the volume is far too large for row-by-row work.
  ...
  FROM dialer_calls
  WHERE lead_external_id IS NOT NULL AND start_time < p_cutoff;

  SELECT count(*) INTO v_metadata
  FROM dialer_calls
  WHERE start_time < p_cutoff
  ...
  IF NOT p_dry_run THEN
    ...
    UPDATE dialer_calls
    SET lead_external_id = NULL, updated_at = now()
    WHERE lead_external_id IS NOT NULL AND start_time < p_cutoff;
...
REVOKE ALL ON FUNCTION public.gdpr_clean_dialer_calls(timestamptz, text[], boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.gdpr_clean_dialer_calls(timestamptz, text[], boolean) TO service_role;

COMMENT ON FUNCTION public.gdpr_clean_dialer_calls(timestamptz, text[], boolean) IS
'GDPR: clears recording links, dialer lead keys and non-allowlisted metadata on dialer calls older than the cutoff. Statistical fields are never touched. Called by the daily gdpr-data-cleanup job.';

CREATE INDEX IF NOT EXISTS idx_dialer_calls_start_time_recording
  ON public.dialer_calls (start_time)
  WHERE recording_url IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_dialer_calls_start_time_lead
  ON public.dialer_calls (start_time)
