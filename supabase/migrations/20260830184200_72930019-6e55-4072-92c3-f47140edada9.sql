-- ============================================================================
-- A1: Haerd historisk teamattribution FOER oprydning af team_members
-- ============================================================================

-- 1) sync_last_team_id: maa ALDRIG nulstille last_team_id.
...
BEGIN
  ...
  ELSIF TG_OP = 'DELETE' THEN
    -- Behold en evt. anden teamtilknytning, ellers bevar det team medarbejderen
    -- netop forlod. Saet ALDRIG NULL - vaerdien bruges som historisk fallback.
    UPDATE public.employee_master_data
    SET last_team_id = COALESCE(
...
-- 2) fn_remove_assignments_on_team_member_delete: ryd IKKE klienttilknytninger
--    for inaktive medarbejdere (historik for timeregistrering/rapporter).
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE e.id = OLD.employee_id AND e.is_active = true
  ) THEN
    RETURN OLD;
  END IF;

  DELETE FROM public.employee_client_assignments eca
  WHERE eca.employee_id = OLD.employee_id
    AND eca.client_id IN (
      SELECT tc.client_id FROM public.team_clients tc WHERE tc.team_id = OLD.team_id
    )
    AND NOT EXISTS (
      SELECT 1 FROM public.employee_master_data e
...
-- 3) get_league_team_provision: historisk provision skal fortsat kobles til det
--    team saelgeren sad paa, ogsaa efter medarbejderen er deaktiveret.
...
-- 4) get_team_performance_summary: historisk attribution via last_team_id,
--    og emp_count taeller kun aktive medarbejdere.
...
BEGIN
  ...
  -- Teamtilknytning til historisk attribution: aktive medlemsskaber + sidste
  -- kendte team for inaktive medarbejdere.
  team_emp AS (
    SELECT tm.team_id, tm.employee_id
    FROM team_members tm
    JOIN active_teams at ON at.id = tm.team_id
    UNION
    SELECT e.last_team_id AS team_id, e.id AS employee_id
    FROM employee_master_data e
    JOIN active_teams at2 ON at2.id = e.last_team_id
    WHERE e.is_active = false AND e.last_team_id IS NOT NULL
  ),
  -- Antal medarbejdere pr. team: kun AKTIVE medarbejdere.
  team_emp_count AS (
    SELECT tm.team_id, COUNT(*)::int AS emp_count
    FROM team_members tm
    JOIN active_teams at3 ON at3.id = tm.team_id
    JOIN employee_master_data e ON e.id = tm.employee_id AND e.is_active = true
    GROUP BY tm.team_id
  ),
