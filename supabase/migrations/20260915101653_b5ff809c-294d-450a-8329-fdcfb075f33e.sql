-- Fælles produktoptælling for opstartsmodulet (single source of truth)
CREATE OR REPLACE FUNCTION public.ramp_product_count(
  p_campaigns uuid[],
  p_email text,
  p_from date,
  p_to date
...
AS $function$
  SELECT coalesce(sum(
           CASE WHEN coalesce(si.is_cancelled, false) THEN 0
                ELSE greatest(coalesce(si.quantity, 1) - coalesce(si.cancelled_quantity, 0), 0)
           END), 0)::int
  FROM public.sale_items si
  JOIN public.sales sa ON sa.id = si.sale_id
  JOIN public.products p ON p.id = si.product_id
  WHERE sa.client_campaign_id = ANY(p_campaigns)
    AND sa.agent_email = p_email
    AND (sa.sale_datetime AT TIME ZONE 'Europe/Copenhagen')::date BETWEEN p_from AND p_to
    AND p.counts_as_sale = true
...
BEGIN
  ...
  daily AS (
    SELECT d.id, d.day_no,
           public.ramp_product_count(v_campaigns, d.email, d.work_date, d.work_date) AS sales_count
    FROM days d
...
BEGIN
  ...
  measured AS (
    SELECT p.day_no, p.stopped, p.pending,
           public.ramp_product_count(v_campaigns, p.email, p.date1, p.measure_date) AS cum_sales,
...
BEGIN
  ...
  daily AS (
    SELECT d.day_no,
           public.ramp_product_count(v_campaigns, v_enroll.email, d.work_date, d.work_date) AS sales_count
    FROM days d
...
GRANT EXECUTE ON FUNCTION public.ramp_product_count(uuid[], text, date, date) TO authenticated, service_role;