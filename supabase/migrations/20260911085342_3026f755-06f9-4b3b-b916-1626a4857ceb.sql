ALTER TABLE public.employee_master_data
  ADD COLUMN IF NOT EXISTS sales_history_from date NULL;

COMMENT ON COLUMN public.employee_master_data.sales_history_from IS
  'Bruges UDELUKKENDE til at tilskrive salgshistorik i employee_profile_stats. Findes til genansættelser, hvor employment_start_date er den seneste ansættelse og ikke den første. Har ingen betydning for løn, provision eller adgang.';
...
BEGIN
  ...
  -- Salg før medarbejderens skæringsdato tilskrives ikke. Findes hverken
  -- sales_history_from eller employment_start_date, tilskrives alt som før.
  ...
  marked AS (
    -- En dag springes kun over ved godkendt fravær eller weekend.
    -- En hverdag uden salg bryder striben, også hvis der ikke findes en planlagt vagt.
    SELECT c.emp, c.d,
