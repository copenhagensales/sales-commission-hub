...
-- Drop the overloaded versions and recreate both with OPP extracted from raw_payload
...
-- Recreate the 3-param version
CREATE OR REPLACE FUNCTION public.get_sales_report_raw(p_client_id uuid, p_start text, p_end text)
RETURNS TABLE(employee_name text, sale_datetime timestamptz, product_name text, quantity numeric, commission numeric, revenue numeric, customer_phone text, customer_company text, status text, internal_reference text, adversus_opp_number text)
...
-- Recreate the 5-param version (with pagination)
