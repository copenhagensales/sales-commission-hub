-- Drop and recreate function with counts_as_cross_sale column
DROP FUNCTION IF EXISTS public.get_aggregated_product_types();

CREATE FUNCTION public.get_aggregated_product_types()
 RETURNS TABLE(adversus_external_id text, adversus_product_title text, product_id uuid, product_name text, commission_dkk numeric, revenue_dkk numeric, product_client_campaign_id uuid, counts_as_sale boolean, counts_as_cross_sale boolean, is_hidden boolean, client_id uuid, client_name text, sale_source text)
 LANGUAGE sql
