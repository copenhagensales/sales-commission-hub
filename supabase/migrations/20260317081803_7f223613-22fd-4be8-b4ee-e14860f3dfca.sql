...
-- Add hired_employee_id column to link referrals directly to the hired employee
ALTER TABLE public.employee_referrals
ADD COLUMN IF NOT EXISTS hired_employee_id UUID REFERENCES public.employee_master_data(id) ON DELETE SET NULL;

-- Update deactivation trigger to auto-reject referrals when employee leaves before 60 days
