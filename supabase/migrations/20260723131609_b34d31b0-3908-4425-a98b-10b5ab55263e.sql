...
RETURNS TABLE(
  employee_name text, sale_datetime timestamp with time zone, product_name text, campaign_name text,
  location_name text, quantity numeric, commission numeric, revenue numeric,
  customer_phone text, customer_company text, status text, internal_reference text,
  adversus_opp_number text, cvr_number text
)
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(cc_sale.name, cc_prod.name, cc_mapping.name) AS campaign_name,
    loc_lookup.location_name AS location_name,
    ...
    (SELECT elem->>'value'
       FROM jsonb_array_elements(COALESCE(s.raw_payload->'leadResultData','[]'::jsonb)) elem
       WHERE elem->>'label' = 'Sales ID'
       LIMIT 1) AS cvr_number
  FROM sale_items si
