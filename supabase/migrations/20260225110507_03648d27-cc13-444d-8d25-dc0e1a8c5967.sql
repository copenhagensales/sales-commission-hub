...
BEGIN
  ...
  SELECT
    ...
    END AS group_key,
    CASE
      WHEN p_group_by = 'employee' THEN COALESCE(a.name, emd_fb.first_name || ' ' || emd_fb.last_name, s.agent_email)
      WHEN p_group_by = 'date' THEN (s.sale_datetime AT TIME ZONE 'Europe/Copenhagen')::date::text
      WHEN p_group_by = 'both' THEN COALESCE(a.name, emd_fb.first_name || ' ' || emd_fb.last_name, s.agent_email) || ' (' || (s.sale_datetime AT TIME ZONE 'Europe/Copenhagen')::date::text || ')'
      ELSE 'Total'
    END AS group_name,
    COALESCE(SUM(
  ...
  -- Fallback: match directly on work_email when no agent mapping exists
