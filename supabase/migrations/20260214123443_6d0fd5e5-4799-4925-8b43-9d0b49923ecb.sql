...
BEGIN
  ...
  WITH filtered_sales AS (
    SELECT 
      ...
      si.mapped_revenue,
      p.counts_as_sale,
      eam.employee_id AS mapped_employee_id
    FROM sale_items si
  ...
  SELECT 
    CASE 
      WHEN p_group_by = 'employee' THEN COALESCE(fs.mapped_employee_id::TEXT, fs.agent_email)
      WHEN p_group_by = 'date' THEN DATE(fs.sale_datetime)::TEXT
      WHEN p_group_by = 'both' THEN COALESCE(fs.mapped_employee_id::TEXT, fs.agent_email) || '|' || DATE(fs.sale_datetime)::TEXT
      ELSE 'total'
    END AS group_key,
    CASE 
