CREATE OR REPLACE FUNCTION public.get_churn_30d_monthly_trend(p_as_of_date date DEFAULT NULL::date, p_months integer DEFAULT 6, p_horizon_days integer DEFAULT 30)
RETURNS jsonb
LANGUAGE plpgsql
...
DECLARE
  ...
  v_as_of date;
  v_horizon integer := CASE WHEN coalesce(p_horizon_days, 30) IN (30, 60, 90) THEN p_horizon_days ELSE 30 END;
...
REVOKE ALL ON FUNCTION public.get_churn_30d_monthly_trend(date, integer, integer) FROM public;
GRANT EXECUTE ON FUNCTION public.get_churn_30d_monthly_trend(date, integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_churn_30d_monthly_trend(date, integer, integer) TO service_role;