-- 1) Ny handlingstype: fravær hele ugen
ALTER TABLE public.ramp_flag_action DROP CONSTRAINT ramp_flag_action_action_type_check;
ALTER TABLE public.ramp_flag_action ADD CONSTRAINT ramp_flag_action_action_type_check
  CHECK (action_type = ANY (ARRAY[
    '1-1 samtale'::text,
    'medlyt med feedback'::text,
    'fravær hele ugen'::text,
    'ny leadbatch'::text,
    'samtale med salgschef'::text,
    'ingen handling'::text
  ]));

-- 2) Faste eskaleringsmodtagere (mønster fra compliance_alert_recipients)
CREATE TABLE public.ramp_escalation_recipients (
...
GRANT SELECT ON public.ramp_escalation_recipients TO authenticated;
GRANT INSERT, UPDATE, DELETE ON public.ramp_escalation_recipients TO authenticated;
GRANT ALL ON public.ramp_escalation_recipients TO service_role;

ALTER TABLE public.ramp_escalation_recipients ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Alle indloggede kan se ramp-eskaleringsmodtagere"
  ON public.ramp_escalation_recipients FOR SELECT TO authenticated USING (true);
CREATE POLICY "Kun superadmin kan oprette ramp-eskaleringsmodtagere"
  ON public.ramp_escalation_recipients FOR INSERT TO authenticated WITH CHECK (public.am_i_superadmin());
CREATE POLICY "Kun superadmin kan rette ramp-eskaleringsmodtagere"
  ON public.ramp_escalation_recipients FOR UPDATE TO authenticated
  USING (public.am_i_superadmin()) WITH CHECK (public.am_i_superadmin());
CREATE POLICY "Kun superadmin kan slette ramp-eskaleringsmodtagere"
  ON public.ramp_escalation_recipients FOR DELETE TO authenticated USING (public.am_i_superadmin());

CREATE TRIGGER update_ramp_escalation_recipients_updated_at
  BEFORE UPDATE ON public.ramp_escalation_recipients
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();

INSERT INTO public.ramp_escalation_recipients (employee_id, is_active)
VALUES ('f6d3dcde-81ef-498f-b4ac-8da218408eee', true),
       ('4929cfad-230f-4521-82a5-0affee2bdb8f', true)
ON CONFLICT (employee_id) DO NOTHING;

-- 3) Arbejdsdage for en saelger i en given uge (hverdage minus danske helligdage,
--    afgraenset af opstartsdato og evt. fratraedelsesdato)
CREATE OR REPLACE FUNCTION public.ramp_week_workdays(
  p_start date,
  p_monday date,
...
AS $$
  SELECT count(*)::int
  FROM generate_series(greatest(p_start, p_monday)::timestamp,
                       least(p_monday + 6, coalesce(p_end, p_monday + 6))::timestamp,
                       interval '1 day') d
  WHERE extract(isodow FROM d) < 6
    AND NOT EXISTS (SELECT 1 FROM public.danish_holiday h WHERE h.date = d::date)
$$;

-- 4) Oversigt til lederens side, nu med ugesalg, typisk spaend pr. uge og ugens forloeb
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
           ...
           'week_required', (d.workdays_this_week >= 2 AND NOT d.has_absence),
...
-- 5) Payload til fredagens paamindelse: hvem mangler ugens forloeb
