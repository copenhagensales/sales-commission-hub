...
3. Migration opdaterer cron-forskydning samt persisterer `sync_days = 3` for ASE.
...
3. Verificér at cron schedules er opdateret i `cron.job`.
...
3. Fjern `config.sync_days` override for ASE hvis nødvendigt:

```sql
update public.dialer_integrations
set config = coalesce(config, '{}'::jsonb) - 'sync_days',
    updated_at = now()
