...
export function ChurnTrendChart30Days() {
  const { data: chartData, isLoading } = useQuery({
    queryKey: ["churn-trend-chart-30-days-data"],
    queryFn: async () => {
      ...
      // For each month: employees who STARTED in that month, and what % left within 30 days
      const chartPoints = months.map(({ month, label }) => {
        ...
        // Count exits within 30 days (instead of 60)
        // From historical data
        const historicalExits30 = historicalStartedInMonth.filter(emp => emp.tenure_days <= 30).length;
        
        // From master data (inactive employees who left within 30 days)
        ...
        const churnRate = cohortSize > 0 ? (exits30Days / cohortSize) * 100 : 0;
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>30-dages Churn Udvikling</CardTitle>
                  ...
                  return (
                    <div className="bg-popover border border-border rounded-lg p-3 shadow-lg">
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {data.exits30Days} af {data.cohortSize} nye stoppede inden 30 dage
                      </p>
                    </div>
                  );
                }}
