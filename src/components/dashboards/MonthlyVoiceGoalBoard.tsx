...
import { useCanManageBoardGoals, useSaveBoardMonthlyGoals } from "@/hooks/useBoardMonthlyGoals";
import type { MonthlyVoiceGoalData } from "@/hooks/useVoiceMonthlyGoal";
import { Target, Trophy, Loader2, Pencil } from "lucide-react";
...
export interface MonthlyVoiceGoalBoardProps {
  /** Board-nøgle i `board_monthly_goals`. */
  boardKey: string;
  title: string;
  /** Tekst efter månedsnavnet, fx "voice-salg (alt undtagen 5G Internet)". */
  subtitleSuffix: string;
  /** Tekst når der hverken er salg eller mål. */
  emptyText: string;
  data: MonthlyVoiceGoalData | undefined;
  isLoading: boolean;
  error: unknown;
}

/** Fælles visning for månedsmål-boards med voice-salg (Eesy FM og Eesy TM). */
export function MonthlyVoiceGoalBoard({
  boardKey,
  title,
  subtitleSuffix,
  emptyText,
  ...
  isLoading,
  error,
}: MonthlyVoiceGoalBoardProps) {
  const tv = isTvMode();
  useAutoReload(tv, 5 * 60_000);
  const { data: canManage } = useCanManageBoardGoals();
  const monthKey = data?.monthKey ?? "";
  const saveGoals = useSaveBoardMonthlyGoals(boardKey, monthKey);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast.error(`Kunne ikke gemme målene: ${(e as Error).message}`);
    }
  };
    ...
    <div className="cph-board min-h-screen w-full bg-slate-950 text-white p-6 md:p-10">
      <div className="flex items-center justify-between mb-5">
        <div>
          ...
          <p className="text-slate-400 mt-0.5 text-base">
            {data?.monthLabel ?? ""} · {subtitleSuffix}
          </p>
        </div>
      ...
      {!tv && canManage && (
        <Dialog open={editOpen} onOpenChange={setEditOpen}>
          <DialogContent className="max-w-lg">
            <DialogHeader>
              <DialogTitle>Redigér mål — {data?.monthLabel ?? ""}</DialogTitle>
              <DialogDescription>
                Målene gælder kun for denne måned. Sæt 0 for at fjerne et mål.
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
              <div className="flex items-center justify-between gap-4">
                <span className="font-medium">Fælles mål</span>
                <Input
                  ...
                  data!.sellers.map((s) => (
                    <div key={s.employeeId} className="flex items-center justify-between gap-4">
                      ...
                      <Input
                        className="w-28 text-right"
                        inputMode="numeric"
                        value={sellerInputs[s.employeeId] ?? ""}
                        onChange={(e) =>
                          setSellerInputs((prev) => ({ ...prev, [s.employeeId]: e.target.value }))
                        }
                      />
                    </div>
                  ))
            ...
            <DialogFooter>
              ...
              <Button onClick={handleSave} disabled={saveGoals.isPending}>
                {saveGoals.isPending ? "Gemmer..." : "Gem mål"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
  ...
  return (
    <DashboardShell>
      <DashboardHeader title={title} subtitle={data?.monthLabel ?? ""} />
      {content}
    </DashboardShell>
  );
}
