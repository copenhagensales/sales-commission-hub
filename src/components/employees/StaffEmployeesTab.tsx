...
import { Plus, Pencil, Search, Users, Phone, MessageSquare, Loader2, ArrowRight, Check, FileText, Trash2, Eye, EyeOff, Mail, UserCheck, Send } from "lucide-react";
...
type StaffEmployeeRecord = Database["public"]["Tables"]["employee_master_data"]["Row"];
...
export function StaffEmployeesTab() {
  ...
  const { data: staffEmployees = [], isLoading } = useQuery({
  ...
  const saveMutation = useMutation({
    mutationFn: async (employee: NewStaffEmployee & { id?: string }) => {
      const dataToSave = { ...employee, is_staff_employee: true };
      if (employee.id) {
        const { error } = await supabase
          .from("employee_master_data")
          .update(dataToSave)
        ...
        const { error } = await supabase.from("employee_master_data").insert(dataToSave);
        if (error) throw error;
      }
    },
    onSuccess: () => {
      ...
      setFormData(defaultStaffEmployee);
    },
    onError: (error) => {
      toast({ title: t("employees.toast.error"), description: error.message, variant: "destructive" });
    },
  ...
  const handleEdit = (employee: StaffEmployeeRecord) => {
  ...
  const autoSaveEmployee = useCallback(async () => {
    ...
    try {
      const { error } = await supabase
        .from("employee_master_data")
        .update({ ...formData, is_staff_employee: true })
  ...
  const handleSendInvitation = async (employee: StaffEmployeeRecord) => {
    ...
    <div className="space-y-6">
      <Dialog open={dialogOpen} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editingEmployee ? t("employees.dialog.editEmployee") : "Ny stabsmedarbejder"}</DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Select value={formData.salary_type || "fixed"} onValueChange={(v) => setFormData({ ...formData, salary_type: v as "provision" | "fixed" | "hourly" })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl bg-card/50 p-4">
          ...
          <div className="text-2xl font-bold">{staffEmployees.length - activeCount}</div>
        </div>
        <div className="rounded-xl bg-card/50 p-4">
          <div className="flex items-center justify-between mb-2">
