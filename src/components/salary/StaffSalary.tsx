...
export function StaffSalary() {
  ...
  const { data: salaries = [], isLoading } = useQuery({
    queryKey: ["personnel-salaries", "staff"],
    queryFn: async () => {
  ...
  useSalaryAccessAudit(salaries.map((s) => s.employee_id), "personnel_salary_staff");
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["personnel-salaries"] });
      toast({ title: "Stabsmedarbejder fjernet fra lønlisten" });
    },
    onError: () => {
      ...
      <CardContent>
        {salaries.length === 0 && !isLoading ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-lg font-medium">Ingen stabsmedarbejdere tilføjet</p>
            <p className="text-sm">Klik på "Tilføj stabsmedarbejder" for at komme i gang.</p>
          </div>
        ) : (
          <>
            <div className="flex items-center gap-4 mb-4">
              <div className="relative flex-1 max-w-sm">
                ...
                <Input
                  placeholder="Søg stabsmedarbejder..."
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="pl-10"
                />
              </div>
              <Badge variant="secondary">{salaries.length} stabsmedarbejdere</Badge>
            </div>
