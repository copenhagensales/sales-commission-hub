...
import { useEditAbsenceRequest, useDanishHolidays, AbsenceRequest } from "@/hooks/useShiftPlanning";
...
export function EditAbsenceDialog({
  ...
  const editAbsence = useEditAbsenceRequest();
  ...
  useEffect(() => {
    if (absence) {
      ...
      setStartTime(absence.start_time?.slice(0, 5) || "08:00");
      setEndTime(absence.end_time?.slice(0, 5) || "12:00");
  ...
  useEffect(() => {
    if (absence?.type === "vacation" && startDate) {
      ...
      if (daysUntilStart < 35) {
        setShortNoticeWarning(
          `Ferie skal søges med minimum 5 ugers varsel (35 dage). Du kan ikke søge ferie med så kort varsel.`
        );
      } else {
  ...
  }, [absence?.type, startDate]);

  // Check for holiday conflicts
  useEffect(() => {
    if (absence?.type === "vacation" && startDate && holidays) {
      const today = new Date();
      const daysUntilStart = differenceInDays(startDate, today);
      ...
      if (holidayConflict && daysUntilStart < 7) {
        setHolidayWarning(
          `${holidayConflict.name} falder i den valgte periode. Du skal ansøge mindst 7 dage i forvejen.`
        );
      } else {
  ...
  }, [absence?.type, startDate, endDate, holidays]);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Redigér anmodning</DialogTitle>
          <DialogDescription>
            Redigér din {getTypeLabel(absence.type).toLowerCase()}-anmodning
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={!startDate || !endDate || !!holidayWarning || !!shortNoticeWarning || editAbsence.isPending}
          >
            {editAbsence.isPending ? "Gemmer..." : "Gem ændringer"}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
