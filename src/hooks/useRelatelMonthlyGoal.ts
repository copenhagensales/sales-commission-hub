...
export interface RelatelMonthlyGoalDay {
  /** YYYY-MM-DD */
  date: string;
  day: number;
  count: number;
...
export interface RelatelMonthlyGoalData {
  monthLabel: string;
  goal: RelatelMonthlyGoal | null;
  teamCount: number;
  teamGoal: number;
  teamProgress: number;
  sellers: RelatelMonthlyGoalSeller[];
  days: RelatelMonthlyGoalDay[];
...
/**
 * Antal solgte produktlinjer (sum af sale_items.quantity) på Relatel i indeværende
 * måned, fordelt på aktive sælgere i Relatel-teamet.
 *
 ...
 */
export function useRelatelMonthlyGoal(enabled = true) {
  const now = new Date();
  const { start, end } = monthBounds(now);
  const monthKey = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}`;

  return useQuery({
    queryKey: ["relatel-monthly-goal", monthKey],
    enabled,
    staleTime: 60_000,
    queryFn: async (): Promise<RelatelMonthlyGoalData> => {
      const goal = getRelatelMonthlyGoal(now);
      const warnings: string[] = [];

      let payload: RelatelMonthlyGoalPayload = { sellers: [], items: [] };
      try {
        const res = await tvEdgeFetch(
          `tv-dashboard-data?action=relatel-monthly-goal&start=${encodeURIComponent(start)}&end=${encodeURIComponent(end)}`,
        );
        if (!res.ok) throw new Error(`Hentning fejlede (${res.status})`);
        const json = (await res.json()) as RelatelMonthlyGoalPayload & { error?: string };
      ...
      const days: RelatelMonthlyGoalDay[] = Array.from({ length: daysInMonth }, (_, i) => {
        const dayNum = i + 1;
      ...
      const sellers: RelatelMonthlyGoalSeller[] = employees
