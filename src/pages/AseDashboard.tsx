...
const AseDashboard = () => {
  ...
  const { data: saleItems, isLoading } = useQuery({
    queryKey: ['ase-dashboard-sales', dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
    queryFn: async () => {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-background p-6">
        <DashboardHeader title="ASE – Overblik" subtitle="Salgsdata baseret på produkt mapping" />
