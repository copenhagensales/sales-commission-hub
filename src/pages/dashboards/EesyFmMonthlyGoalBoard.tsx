...
export default function EesyFmMonthlyGoalBoard() {
  ...
  const saveGoals = useSaveBoardMonthlyGoals(EESY_FM_MONTHLY_GOAL_BOARD_KEY, monthKey);
  ...
  const content = (
    <div className="cph-board min-h-screen w-full bg-slate-950 text-white p-6 md:p-10">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight">Eesy FM Månedsmål</h1>
          <p className="text-slate-400 mt-0.5 text-base">
            {data?.monthLabel ?? ""} · voice-salg (alt undtagen 5G Internet)
          </p>
        </div>
        ...
        <>
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-6 md:p-8">
            ...
            {(data?.sellers.length ?? 0) === 0 ? (
              <p className="text-slate-400">Ingen Eesy FM-salg eller mål i denne måned.</p>
            ) : (
              (() => {
  ...
  return (
    <DashboardShell>
      <DashboardHeader title="Eesy FM Månedsmål" subtitle={data?.monthLabel ?? ""} />
      {content}
    </DashboardShell>
  );
}
