...
import { useRelatelMonthlyGoal } from "@/hooks/useRelatelMonthlyGoal";
...
export default function RelatelMonthlyGoalBoard() {
  const tv = isTvMode();
  useAutoReload(tv, 5 * 60_000);
  const { data, isLoading, error } = useRelatelMonthlyGoal();
        ...
        <>
          ...
          <div className="rounded-2xl border border-white/10 bg-slate-900/70 p-6 md:p-8">
            ...
            {(data?.sellers.length ?? 0) === 0 ? (
              <p className="text-slate-400">Ingen aktive sælgere på Relatel-teamet.</p>
            ) : (
              (() => {
  ...
  return (
    <DashboardShell>
      <DashboardHeader title="Relatel Månedsmål" subtitle={data?.monthLabel ?? ""} />
      {content}
    </DashboardShell>
  );
}
