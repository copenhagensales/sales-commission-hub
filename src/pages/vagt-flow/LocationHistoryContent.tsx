  ...
  const { data: bookings, isLoading: loadingBookings } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("booking")
        .select("id, location_id, booked_days, daily_rate_override, placement_id, week_number, year, client_id, client:clients!client_id(name), location!inner(id, name, daily_rate)")
